...
class Payment < ActiveRecord::Base
  ...
  def pay_url(user)
    ...
    Alipay::Service.create_partner_trade_by_buyer_url(
      ...
      :logistics_fee     => '0',
      :logistics_payment => 'SELLER_PAY',
      :return_url        => Rails.application.routes.url_helpers.user_payment_url(user_id: user.id, id: id, protocol: (ENV["SSL"] ? 'https' : 'http'), :host => ENV["HOST_NAME"]),
      :notify_url        => Rails.application.routes.url_helpers.alipay_notify_user_payments(user_id: user.id, protocol: (ENV["SSL"] ? 'https' : 'http'), :host => ENV["HOST_NAME"])
    )
  end
